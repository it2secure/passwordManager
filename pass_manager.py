...
cursor=conn.cursor()
cursor.execute("""CREATE TABLE IF NOT EXISTS manager(app_name text, url text, email_id text, password text)""")
conn.commit()
conn.close()
...
def change():
    ...
    if app_name_edit.get()!="" and url_edit.get()!="" and email_id_edit.get()!="" and password_edit.get()!="":
        ...
        WHERE oid = :oid""",
        {
            ...
            'url': url_edit.get(),'email_id': email_id_edit.get(),'password': password_edit.get(),'oid': update_id.get()})
        conn.commit()
        conn.close()
